...
@section('content')
    ...
    <div class="row m-0 p-0 justify-content-center g-3">
        ...
        <div class="col-6 col-md-4 col-lg-3 col-xl-2 p-1 p-md-3">
            <div class="card h-100 shadow-sm border-0 rounded product-card">
                <img src="{{ $product->photo }}" class="card-img-top rounded-top"
                    style="width: 100%; aspect-ratio: 3/2; object-fit: cover;" alt="{{ $product->name }}">
                <div class="card-body d-flex flex-column p-3 text-center">
                    <h5 class="card-title text-dark fw-bold mb-1" style="font-size: 16px;">{{ $product->name }}</h5>
                    <p class="fw-bold mb-1" style="color: var(--primary-pink);">${{ $product->price }}</p>
                    <p class="card-text text-muted mb-3" style="font-size: 13px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">{{ $product->description }}</p>
                    <div class="mt-auto d-flex flex-column flex-xl-row justify-content-between gap-2">
                        <a href="#"
                            onclick="event.preventDefault(); triggerCart('{{ addslashes($product->name) }}', '{{ $product->photo }}', '{{ addslashes($product->description) }}', '{{ $product->price }}')"
                            class="btn btn-primary theme-btn w-100" style="font-size: 13px;"><i class="bi bi-bag me-1"></i>
                            Cart</a>
                        <a href="#"
                            onclick="event.preventDefault(); triggerWishlist('{{ addslashes($product->name) }}', '{{ $product->photo }}', '{{ addslashes($product->description) }}', '{{ $product->price }}')"
    ...
    <form id="wishlistForm" method="POST" action="/wishlist/add" class="d-none">
        ...
        <input type="hidden" name="product_price" id="wl_price">
        <input type="hidden" name="description" id="wl_desc">
    </form>
    <script>
        function triggerWishlist(name, photo, desc, price = '25.00') {
            document.getElementById('wl_name').value = name;
            document.getElementById('wl_photo').value = photo;
            document.getElementById('wl_price').value = price;
